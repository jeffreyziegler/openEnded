#' @title Plot the ATE for Compliers and Non-Compliers
#' @description 
...
plotComplierATE <- function(dataframe=NULL, similarity_measures=NULL, bounds=NULL, n=100, seed=12345, formulas=NULL, model_type=NULL, k=NULL){
  bootstrappedData <- data.frame()
  
  for(model in 1:length(formulas)){
    for(i in 1:n){
      bootstrappedData <- rbind(bootstrappedData, ATEcutoff(dataframe, similarity_measures, formula=formulas[[model]], model_type, k, bounds))
    }
  }
